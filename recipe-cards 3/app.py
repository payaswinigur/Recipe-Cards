...
def create_app(database_path=None):
    ...
    # On a host with an ephemeral disk (Render's free tier, for one), the
    # database file is gone after every restart. Build and seed it on the
    # way up so the deployed demo is never an empty page, and so anyone who
    # deletes every recipe only does so until the next restart.
    #
    # This is a no-op once the schema exists, so it never touches real data.
